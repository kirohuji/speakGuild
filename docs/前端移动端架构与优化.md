...
| 子系统 | 用途 | 存储介质 |
|--------|------|----------|
| **LearningPack** | 场景、词汇、句块等结构化学习内容 | SQLite |
| **AssetCache** | 音频、图片等二进制资源 | Filesystem |
...
| `learning.store` | ❌ 内存 | 学习单元、商店、签到、下载队列 |
...
5. **键盘弹起**：安装 `@capacitor/keyboard`，管理 resize 和布局
...
interface DownloadTask {
  packId: string; title: string; progress: number  // 0-100
