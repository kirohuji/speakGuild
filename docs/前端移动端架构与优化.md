...
| **UI 框架** | React 19 + TypeScript | 声明式 UI；当前全量类型检查仍有少量历史错误待清理 |
| **样式方案** | Tailwind CSS + shadcn/ui | 语义化设计系统 |
| **状态管理** | Zustand (17 stores, 含 persist) | 全局状态 + localStorage 持久化 |
| **路由** | React Router v7 (Hash 路由) | 兼容静态部署 + Capacitor |
...
| Native Audio | `@capgo/capacitor-native-audio` | 原生音频、后台播放 |
| Media Session | `@capgo/capacitor-media-session` | 锁屏媒体信息与播放控制 |
| Local Notifications | `@capacitor/local-notifications` | 学习提醒与测试通知 |
| Badge | `@capawesome/capacitor-badge` | 今日待完成数量角标 |
...
| `daily-practice.store` | ❌ 内存 | 今日练习计划与执行状态 |
| `warmup-session.store` | ✅ localStorage | Guided warmup 当前题与恢复状态 |
| `practice-activity.store` | ❌ 内存 | 有效练习时长与活动统计 |
| `global-task.store` | ❌ 内存 | 全局下载/后台任务展示 |
| `feature-flags.store` | ✅ localStorage | 客户端功能开关 |
| `app-update.store` | ✅ localStorage | OTA 更新状态 |
