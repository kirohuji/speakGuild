...
> 状态：🟡 本地 warmup judge 与本地 STT 主体已实现；仍需持续做模型兼容、性能和移动端真机回归。
>
> 实现核对：2026-08-03
>
> 边界说明：本地 STT 解决的是“语音转文字”，不等于音素级发音评分。当前 `pronunciationNotes` 属于基于转写文本的教练建议，不能对外描述为声学级发音测评。
...
当前实现入口包括 `lib/local-stt/` 下的模型管理、Worker 和服务层，以及设置页中的模型下载、删除、启用和空间占用管理。录音输入会在满足条件时优先使用本地转写，并保留云端回退路径。
