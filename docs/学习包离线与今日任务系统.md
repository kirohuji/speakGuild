...
> 状态：✅ 离线安装、SQLite 存储、资源池、暂停恢复、卸载和 outbox 同步主体已实现；Delta 包、安装分析和更完整的真机回归仍待推进。
>
> 最后核对：2026-08-03
>
> 统一现状和后续优先级见 [产品功能现状与实施路线图](./产品功能现状与实施路线图.md)。
...
> 已实施：`writing` 写作包、`reading` 阅读包、`listening` 听力包、`novel` 小说包和 `story` 剧情包均不进入今日任务。包内写作题、理解题或听力题属于本模式的学习活动，不会成为 Warmup。Today 候选现按 `contentMode=practice` 白名单筛选，后端完成接口也会重复校验；离线 manifest 与本地聚合均保留 `contentMode`。完整方案见 [多形态学习包统一内容模型与实施方案](./阅读包与AI写作练习包实施方案.md)。
...
排期规则已与学习本统一为 `Again / Hard / Good / Easy` 的 SM-2 状态机。客户端为了离线使用同一规则先行计算；同步时不再上传整份排期状态，后端根据 `clientAttemptId` 幂等保存 attempt，再按题目历史顺序重放排期并返回权威进度覆盖本地。未同步 attempt 存在时，后台静默拉取不会覆盖本地较新的离线进度。
...
> **相关文档**：[产品功能现状与实施路线图](./产品功能现状与实施路线图.md)；[前端移动端架构与优化](./前端移动端架构与优化.md)；[输出练习链路设计方案](./输出练习链路设计方案.md)
