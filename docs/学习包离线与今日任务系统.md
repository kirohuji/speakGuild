...
> 规划边界：后续新增的 `reading` 阅读包和 `writing` 写作包与现有 `story` 剧情包一样，不进入今日任务。实现时不能继续采用“只排除 `story`”的黑名单，而应把 Today 候选改为包能力白名单，并在后端同步接口重复校验。完整方案见 [阅读包与 AI 写作练习包实施方案](./阅读包与AI写作练习包实施方案.md)。
