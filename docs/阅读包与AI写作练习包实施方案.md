...
- 权限、价格/免费状态、发布状态、版本。
- 无话题包的知识引用：小说包可在 Scene 层绑定单词、句块、句型；有话题或剧情章节的包不得重复绑定。
...
- 练习、写作、阅读、听力包：从 `TrainingTopic` 绑定并在包详情聚合。
- 剧情包：从 `StoryEpisode` 绑定并按剧情包聚合。
- 小说包：因为没有话题或练习章节，才由 Scene 包级绑定。

不得为了商店计数或总复习再给有话题的包增加一套包级绑定，否则同一知识会出现两个来源并破坏原有统计口径。
...
- 仅小说等无话题包显示单词、句块、句型的包级绑定；其他包在话题或剧情章节中维护。
- 资产、版本、构建、发布和离线预览。
...
4. 复用 `TrainingTopic` 和 `StoryEpisode` 的知识绑定；Scene 包级知识只服务无话题的小说包，不新增平行的 ContentUnit 模型。
