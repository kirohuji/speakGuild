...
HEADER = '漫语町（ManYu）沉浸式英语输出训练系统 V1.0  技术文档'
SNAME = '漫语町（ManYu）沉浸式英语输出训练系统 V1.0'
...
def main():
    ...
    H(doc, '1.1  编写目的')
    P(doc, '本文档是漫语町（ManYu）沉浸式英语输出训练系统的技术设计说明书。编写目的是为了全面、系统、准确地描述该软件的技术架构、模块组成、数据库设计、接口规范、安全机制以及核心业务流程，为软件的开发、测试、部署、运维提供技术依据，同时作为计算机软件著作权登记申请的技术证明材料。')
    P(doc, '本文档面向的读者包括：系统设计人员、开发工程师、测试工程师、运维人员以及软件著作权登记审查人员。')
    H(doc, '1.2  项目背景')
    P(doc, '随着全球化进程的加速和跨文化交流的日益频繁，英语作为国际通用语言的实用能力需求日益旺盛。沉浸式英语输出训练是提升英语口语表达能力和思维能力的有效途径。传统英语学习方式偏重阅读和语法，缺乏真实场景的口语输出训练机会，学习者难以将被动知识转化为主动表达能力。')
    P(doc, '基于上述背景，本项目致力于打造一款沉浸式英语输出训练系统，通过场景驱动、语块积累和AI互动反馈，帮助学习者在真实语境中高效提升英语口语输出能力。')
    H(doc, '1.3  适用范围')
    P(doc, '本系统适用于需要进行英语口语输出训练的学习者，覆盖从初级到高级不同英语水平的使用者。系统支持Web浏览器和iOS移动端两种使用方式，满足学习者在不同场景下的学习需求。')

    # ═══════ 2 系统概述 ═══════
    np(doc, pg); pg += 1; H(doc, '2  系统概述')
    H(doc, '2.1  系统定位')
    P(doc, '漫语町（ManYu）是一款沉浸式英语输出训练系统。系统以"场景驱动、语块积累、互动输出"为核心学习理念，围绕场景学习、口语练习、互动剧本、AI反馈的学习闭环，提供场景化学习、口语练习、模拟考试、AI发音反馈、TTS语音合成、互动剧本探险等完整功能链。系统采用SaaS模式运营，支持会员订阅制，管理员可通过Web后台进行内容管理和运营配置。')
    H(doc, '2.2  核心功能')
