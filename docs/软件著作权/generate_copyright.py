...
doc.add_paragraph()
add_para('软件名称：漫语町（ManYu）多语种导游资格面试练习系统', bold=True, align=WD_ALIGN_PARAGRAPH.CENTER, size=Pt(14))
...
add_para(
    '漫语町（ManYu）是一款面向全国导游资格外语面试考生的沉浸式在线练习与备考辅助系统。'
    '以"场景驱动、语块积累、互动输出"为核心学习理念，将导游真实工作场景融入练习流程，'
    '帮助学习者在沉浸式互动中高效掌握外语导游技能。随着我国入境旅游市场的持续增长，'
    '多语种导游人才需求日益旺盛。然而，传统备考方式面临以下痛点：'
)
bullets = [
    '模拟考试资源匮乏，考生难以获得真实考试场景体验',
    '口语练习缺乏即时反馈，发音问题无法及时发现和纠正',
    '题库覆盖面有限，难以满足不同省份、语种、考试类型的差异化需求',
    '学习进度与效果缺乏量化追踪，备考效率难以提升',
]
for b in bullets:
    doc.add_paragraph(b, style='List Bullet')

add_para(
    '基于上述背景，本系统旨在通过数字化手段和游戏化设计，为外语导游考生提供一站式的沉浸式备考解决方案。'
    '系统整合了全国31个省市自治区的导游资格面试大纲内容，'
    '支持多语种（英语、日语、俄语等）的题库浏览、场景化学习、口语练习、AI发音反馈、'
    'TTS语音合成、模拟考试、互动剧本探险等核心功能，帮助考生高效备考、精准提升。'
)
...
bullets2 = [
    ...
    '源程序代码的页眉格式为："漫语町（ManYu）多语种导游资格面试练习系统 V1.0  源代码  第 X 页"',
]
for b in bullets2:
    doc.add_paragraph(b, style='List Bullet')
...
bullets3 = [
    ...
    '操作手册的页眉格式为："漫语町（ManYu）多语种导游资格面试练习系统 V1.0  操作手册  第 X 页"',
]
for b in bullets3:
    doc.add_paragraph(b, style='List Bullet')
