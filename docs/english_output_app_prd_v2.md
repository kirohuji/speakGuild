...
| 模块 | 状态 | 说明 |
|------|------|------|
| 练习模式 | ✅ 已实现 | 场景→话题→词汇预热→Chunk 激活→录音→AI 纠错→表达升级→复述→保存 |
| 剧本模式 | ✅ 已实现 | Ink 叙事驱动 + AI 任务判断，Chapter 0-1 内容已就绪 |
| 探索模式 | ✅ 已实现 | 地图→地点→NPC + Ink 剧本 + 自由对话 fallback + 游戏存档 |
| 表达库 | ✅ 已实现 | 错句/Chunk/升级表达 CRUD + 间隔复习 |
| 等级系统 | ✅ 已实现 | 用户等级 (XP) + 输出等级 (L1-L5) + 场景熟练度 + Chunk 掌握度 |
| 成就系统 | ✅ 已实现 | 事件驱动检测引擎 + 里程碑/连续打卡/挑战/隐藏成就 + 稀有度 |
| 学习计划 | ✅ 已实现 | 教材→学习单元→练习→剧本 驱动路径 |
| 新手引导 | ✅ 已实现 | 目标选择→能力自评→诊断结果 |
| VN 引擎 | ✅ 已实现 | inkjs 叙事 + PIXI.js 渲染 (React fallback 共存) |
| PIXI.js 渲染 | ✅ 已实现 | 背景/立绘/对话框/转场动画 |
| 会员/支付 | ✅ 已有 | 保留原有系统 |
| 管理员后台 | ✅ 已改造 | 新增场景/Chunk/剧本/角色/地图/成就管理页 |
...
```
/                          首页（EnglishHomePage）
/learning                  学习计划（教材→学习单元）
/learning/units/:unitId    学习单元详情
/today                     今日任务
/practice                  练习模式话题选择
/practice/session/:topicId 练习会话
/script                    剧本模式章节列表
/script/:episodeId         剧本对话（VN 引擎）
/explore                   探索模式地图
/explore/:locationId       探索模式地点
/expressions               表达库
/growth                    我的成长
/achievements              成就殿堂
/onboarding/goals          新手引导—目标选择
/onboarding/ability        新手引导—能力自评
```
