...
> 版本：v1.5 | 日期：2026-07-17 | 状态：当前实现对齐稿（学习包 / 话题 VN / 剧情包 / 地图探索）

> **阅读优先级**：§0 是依据当前代码与数据库整理出的最新架构，是后续产品与开发决策的准绳；§1～§9 保留早期内容策划和场景示例，其中“6 大场景”“场景 100% 才解锁地图”等描述属于历史方案，不应再直接映射为当前数据结构。
...
| Exploration VN | 探索对话 / 支线 | `GameMap → GameLocation → GameRoom → NPC` | 不固定 | 通常不做硬评分 | 是 | `ExplorationRecord` / `GameSave` |
...
| `daily` | 必须 | 推荐 | 可选短剧 | 可选 | 高频生活主题可逐步补地图 |
| `exam` | 必须 | 按题型适配 | 通常不需要 | 不需要 | 保持任务和评分导向 |
| `story` | 可选 | 可选 | 必须 | 可选但推荐用于长线剧情 | 剧集是主体，地图是世界导航层 |

最终规则：**练习话题允许只有练习；剧情包必须有 StoryEpisode，但不必有地图；只有需要“地点移动、NPC 常驻、支线和世界状态”的剧情包才建设地图。**
...
底部“场景”Tab 可以承载所有“用英语”的体验，但首屏应按来源分区，而不是混成一个无限 VN 列表：
...
- 地图地点：`requiredOutputLevel + requiredSceneIds + requiredFlags`。
...
当前代码已经具备 `TrainingTopic`、`StoryEpisode`、三级地图、Ink、Pixi-VN 和视频时间轴；主要缺口是“归属约束与发布模型”，不是播放器本身。
...
4. **剧情包再接地图**：先让剧情包以章节/剧集独立可用；只有确实需要探索的包再创建 Map → Location → Room → NPC。
5. **最后扩展内容量**：在目录、归属校验、资源完整性和发布状态完善后，再批量开放 VN，避免产生大量孤儿脚本。

### 0.10 后台页面落地（2026-07-17）
...
- **剧情共享资产**（`/admin/narrative-assets`）：全局维护角色资产、音色资产和地图世界，供多个剧情包、章节和练习 VN 复用。TTS 厂商凭证与默认模型仍由 `/admin/ai-models` 管理；音色资产通过厂商接口同步或手工录入。
- “地图世界”后台采用两级 Pixi 所见即所得编辑：世界地图以 `GameMap.backgroundUrl` 为底图、`GameLocation.icon + posX/posY` 为地点图片热点；进入地点后，以 `GameLocation.backgroundUrl` 为场景底图、`GameRoom.icon` 为房间图片热点。房间热点布局保存在现有 `GameMap.editorData.explorationScenes` JSON 中，不新增数据库表。背景、热点、悬停高亮和标签在运行时分层渲染，但后台不暴露通用图层、Prefab、碰撞体等引擎概念。
- 地图后台前端按职责拆分：`MapsTab` 负责数据编排，`ExplorationEditorToolbar` 负责层级导航，`ExplorationResourceList` 负责地点/房间目录，`ExplorationPixiCanvas` 与 `HotspotNode` 负责渲染和拖拽，`ExplorationInspector` 负责选中对象信息，`exploration-map-model` 负责 `editorData` 的读写与默认布局。Pixi 组件不直接调用 API。
