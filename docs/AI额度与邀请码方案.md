...
> 日期：2026-06-14
> 状态：已实施
> 定价：**¥19/月 | ¥198/年（iOS only，Apple 15% 抽成后到手 ¥16.15/月）**
> 🎯 第一阶段目标：**100 付费用户，MRR ¥1,615/月**
...
| 阶段 | 时间 | 付费用户 | 月到手 |
|------|:--:|:--:|:--:|
| 冷启动 | 第 1-2 月 | 5-15 | ¥81-242 |
| 增长 | 第 3-4 月 | 30-50 | ¥485-808 |
| 初步验证 | 第 5-6 月 | 60-80 | ¥969-1,292 |
| 🎯 **达标** | **第 6-8 月** | **100** | **¥1,615** |
...
```
月到手：100 × ¥16.15 = ¥1,615
年付占比 30%：30 × ¥168.30 / 12 ≈ ¥421
实际月到手：70 × ¥16.15 + ¥421 ≈ ¥1,551
AI 成本：100 × 不限次 × ¥0.004 × 20%使用率 ≈ ¥120
服务器：~¥300
─────────────
月净利润：~¥1,100
```
...
用户请求 AI 对话判定
  → 查会员等级
    ...
    → 免费：查今日用量
      → 未满 5 次 → 扣减 → 调用 AI
      → 已满 5 次 → 返回提示：「今日额度用完，10积分换1次，或 ¥19 开通会员无限畅练」
```
...
model AiUsageDaily {
  ...
  dialogue    Int  @default(0)  // 对话判定（纠错+对话判定合并）
  summary     Int  @default(0)  // 汇总分析
...
> 实际实现中「纠错」和「对话判定」合并为 `dialogue` 类型，统一走 `free_ai_corrections` 配置控制。
...
| 3 | `PracticeAiController` 加配额检查 | ✅ 已完成 |
| 4 | 邀请奖励：5天会员 + 积分发放 | ✅ 已完成 |
| 5 | `LearningService` 加会员过滤 | ✅ 已完成 |
| 6 | 定价：¥19/月，¥198/年 | ✅ 已完成 |
| 7 | 前 100 名注册免费 5 天试用 | ✅ 已完成 |
| 8 | 价格动态读取 system_config | ✅ 已完成 |
