...
import { listAiProviders, listTtsProviderCatalog, type AiProviderItem } from '../api-ai-models'
...
export function VoiceAssetsTab() {
  ...
  const load = useCallback(async () => {
    const [groups, result] = await Promise.all([
      (isCreator
        ? listTtsProviderCatalog().then((tts) => ({ tts }))
        : listAiProviders()).catch(() => ({ tts: [] } as any)),
      listTtsVoices({
  ...
  }, [isCreator, page, pageSize, providerFilter, search])
  ...
  return <div className="space-y-4">
    <div className="flex flex-wrap items-center gap-3">
      ...
      <span className="text-sm text-muted-foreground">{total} 个音色</span>
      {!isCreator && <Button size="sm" variant="outline" className="ml-auto" disabled={syncing} onClick={() => void sync()}>{syncing ? <Loader2 className="mr-1 size-4 animate-spin" /> : <RefreshCw className="mr-1 size-4" />}同步厂商音色</Button>}
      <Button size="sm" onClick={() => edit()}><Plus className="mr-1 size-4" />手动添加</Button>
    </div>

    {filtered.length ? <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{filtered.map((voice) => <Card key={voice.id} className="overflow-hidden"><CardContent className="p-4"><div className="flex items-start gap-3"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-emerald-500/10 text-emerald-600"><Volume2 className="size-5" /></div><div className="min-w-0 flex-1"><div className="flex items-start gap-2"><div className="min-w-0 flex-1"><p className="truncate font-semibold">{voice.displayName}</p><p className="truncate font-mono text-[11px] text-muted-foreground">{voice.externalVoiceId}</p>{isAdmin && voice.owner && <p className="truncate text-[11px] text-muted-foreground">创作者：{voice.owner.name || voice.owner.email}</p>}</div><Button size="icon" variant="ghost" className="size-7" aria-label="编辑音色" onClick={() => edit(voice)}><Edit3 className="size-3.5" /></Button><Button size="icon" variant="ghost" className="size-7" aria-label="删除音色" onClick={() => void remove(voice)}><Trash2 className="size-3.5 text-destructive" /></Button></div><div className="mt-3 flex flex-wrap gap-1.5"><Badge variant="secondary">{voice.provider.label}</Badge><Badge variant="outline">{voice.category}</Badge>{voice.language && <Badge variant="outline">{voice.language}</Badge>}{voice._count?.characterBindings ? <Badge variant="outline">{voice._count.characterBindings} 个角色引用</Badge> : null}</div></div></div></CardContent></Card>)}</div> : <div className="rounded-xl border border-dashed py-16 text-center text-sm text-muted-foreground">还没有匹配的音色资产</div>}

    <AdminPagination total={total} page={page} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} />
